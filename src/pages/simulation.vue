				...
				<div class="model-section">
					...
					<div class="model-container" ref="sceneContainer">
						...
						<TresCanvas v-if="cityFBX && cityFBX.model && !renderError" ref="tcRef" v-bind="gl" :antialias="true" @error="handleRenderError">
							...
							<!-- 热力图模式下的建筑物模型 - 使用紫色着色器 -->
							<buildingsModel v-if="cityFBX.model && showHeatmap" :model="cityFBX" />
...
import { disposeHeatmapGeometry } from '../plugins/digitalCity/common/heatmapUtils'
import heatmapJS from '../plugins/heatMap/components/heatmapJS.vue'
import buildingsLines from '../plugins/digitalCity/components/buildings/buildingsLines.vue'
import buildingsModel from '../plugins/digitalCity/components/buildings/buildingsModelCustomShader.vue'
...
const timePoints = ref([
	{ value: 0, label: '15:00 (0分钟)' },
	{ value: 1, label: '15:02 (2分钟)' },
	{ value: 2, label: '15:04 (4分钟)' },
	{ value: 3, label: '15:06 (6分钟)' },
	{ value: 4, label: '15:08 (8分钟)' },
	{ value: 5, label: '15:10 (10分钟)' },
	{ value: 6, label: '15:12 (12分钟)' },
	{ value: 7, label: '15:14 (14分钟)' },
	{ value: 8, label: '15:16 (16分钟)' },
	{ value: 9, label: '15:18 (18分钟)' },
	{ value: 10, label: '15:20 (20分钟)' }
])
...
// 热力图数据点接口
interface HeatPoint { x: number; y: number; value: number }

// 经纬度直接转热力图坐标（使用新的转换参数）
const latLonToHeatmapCoords = (lat: number, lon: number, heatmapWidth: number = 256, heatmapHeight: number = 256): { x: number, y: number } => {
	const det = 0.000000002051
	const latDiff = lat - 31.23281069
	const lonDiff = lon - 121.47802229
	const x = (0.00004531 * latDiff - 0.00001537 * lonDiff) / det
	const z = (0.00003951 * lonDiff - -0.00001693 * latDiff) / det
	
	// 热力图坐标就是模型坐标，直接返回并限制在热力图范围内
	const heatmapX = Math.max(1, Math.min(heatmapWidth, Math.round(x)))
	const heatmapY = Math.max(1, Math.min(heatmapHeight, Math.round(z)))
	
	return { x: heatmapX, y: heatmapY }
}

// 热力图坐标转经纬度（用于验证）
const heatmapCoordsToLatLon = (x: number, y: number): { lat: number, lon: number } => {
	const lat = 0.00003951 * x + 0.00001537 * y + 31.23281069
	const lon = -0.00001693 * x + 0.00004531 * y + 121.47802229
	return { lat, lon }
}

// 聚合经纬度数据并生成热力图数据
const aggregateLatLonData = (backendData: Array<{ lat: number, lon: number }>): HeatPoint[] => {
	// 创建经纬度坐标的计数映射
	const coordCounts = new Map<string, { lat: number, lon: number, count: number }>()
	
	// 统计相同经纬度的数量
	backendData.forEach(item => {
		const key = `${item.lat.toFixed(6)}_${item.lon.toFixed(6)}` // 保留6位小数精度
		if (coordCounts.has(key)) {
			coordCounts.get(key)!.count++
		} else {
			coordCounts.set(key, { lat: item.lat, lon: item.lon, count: 1 })
		}
	})
	...
	coordCounts.forEach(({ lat, lon, count }, index) => {
		try {
			// 1. 经纬度直接转热力图坐标
			...
			const latError = Math.abs(verifyLat - lat)
			const lonError = Math.abs(verifyLon - lon)
			
			// 输出详细的转换信息
			console.log(`点${index + 1}: 经纬度(${lat.toFixed(6)}, ${lon.toFixed(6)}) -> 热力图坐标(${x}, ${y}) -> 值=${value} (${count}条记录)`)
			console.log(`  验证: 热力图坐标逆变换 -> 经纬度(${verifyLat.toFixed(6)}, ${verifyLon.toFixed(6)}) 误差(${latError.toFixed(8)}, ${lonError.toFixed(8)})`)
			
			heatmapPoints.push({ x, y, value })
		} catch (error) {
			console.warn(`坐标转换失败 lat=${lat}, lon=${lon}:`, error)
		}
	})
	
	console.log(`聚合结果: ${backendData.length}条原始数据 -> ${coordCounts.size}个唯一坐标 -> ${heatmapPoints.length}个热力图点`)
	...
	if (selectedTimePoint.value !== null) {
		...
		if (timePoint) {
			...
			if (res.success && Array.isArray(res.data) && res.data.length > 0) {
				...
				// 分析经纬度数据分布
				const latValues = res.data.map((item: any) => item.lat).filter((lat: any) => lat != null)
				const lonValues = res.data.map((item: any) => item.lon).filter((lon: any) => lon != null)
				console.log(`\n📊 === 经纬度数据分布 ===`)
				console.log(`纬度范围: ${Math.min(...latValues).toFixed(6)} ~ ${Math.max(...latValues).toFixed(6)}`)
				console.log(`经度范围: ${Math.min(...lonValues).toFixed(6)} ~ ${Math.max(...lonValues).toFixed(6)}`)
				console.log(`前5个经纬度样本:`)
				res.data.slice(0, 5).forEach((item: any, i: number) => {
					console.log(`  ${i + 1}. lat=${item.lat}, lon=${item.lon}`)
				})
				
				// 3. 聚合经纬度数据并转换为热力图坐标
				const heatmapPoints = aggregateLatLonData(res.data)
				
				if (heatmapPoints.length > 0) {
					console.log(`\n🔥 === 热力图数据生成 ===`)
					console.log(`生成热力图点数: ${heatmapPoints.length}`)
					console.log('热力图数据样本:', heatmapPoints.slice(0, 3))
					
					// 更新仿真状态
					simulationStatus.dataPointCount = heatmapPoints.length
					
					// 4. 挂载热力图组件并设置数据
					showHeatmap.value = true
					await nextTick()
					
					if (heatmapJSRef.value && heatmapJSRef.value.setData) {
						heatmapJSRef.value.setData(heatmapPoints)
					}
					...
					console.log(`热力图数据范围: ${heatmapData.value.min} - ${heatmapData.value.max}`)
					return
				} else {
					console.warn('坐标转换后无有效热力图点')
				}
			} else {
			...
			simulationStatus.dataPointCount = 0
